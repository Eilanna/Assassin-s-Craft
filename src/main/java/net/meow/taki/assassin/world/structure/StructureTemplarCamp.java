...
public class StructureTemplarCamp extends ElementsMinecrownrebornMod.ModElement {
	public StructureTemplarCamp(ElementsMinecrownrebornMod instance) {
		super(instance, 163);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 4000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("minecrownreborn", "templar_camp_with_knights_v2"));
				if (template == null)
					return;
