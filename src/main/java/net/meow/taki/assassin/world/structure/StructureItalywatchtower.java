...
public class StructureItalywatchtower extends ElementsMinecrownrebornMod.ModElement {
	public StructureItalywatchtower(ElementsMinecrownrebornMod instance) {
		super(instance, 148);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("minecrownreborn", "italian_watchtower"));
				if (template == null)
					return;
