...
public class StructureTombconnorclient extends ElementsMinecrownrebornMod.ModElement {
	public StructureTombconnorclient(ElementsMinecrownrebornMod instance) {
		super(instance, 94);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("minecrownreborn", "connor_tomb"));
				if (template == null)
					return;
