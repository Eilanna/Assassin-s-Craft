...
import net.meow.taki.assassin.world.WorldThethirdcrusade1189;
...
public class StructureNormal_watchtower extends ElementsMinecrownrebornMod.ModElement {
	public StructureNormal_watchtower(ElementsMinecrownrebornMod instance) {
		super(instance, 146);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 4000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("minecrownreborn:nomanlands")))
					biomeCriteria = true;
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("minecrownreborn:italianplains")))
					biomeCriteria = true;
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("minecrownreborn:syrianplains")))
					biomeCriteria = true;
				if (!biomeCriteria)
					continue;
				if (world.isRemote)
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("minecrownreborn", "basic_watchtower"));
				if (template == null)
					return;
