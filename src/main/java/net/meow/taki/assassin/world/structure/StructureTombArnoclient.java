...
public class StructureTombArnoclient extends ElementsMinecrownrebornMod.ModElement {
	public StructureTombArnoclient(ElementsMinecrownrebornMod instance) {
		super(instance, 97);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("minecrownreborn", "arno_tomb"));
				if (template == null)
					return;
