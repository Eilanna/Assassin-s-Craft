...
public class StructureGreatwatchtower extends ElementsMinecrownrebornMod.ModElement {
	public StructureGreatwatchtower(ElementsMinecrownrebornMod instance) {
		super(instance, 191);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 3000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("minecrownreborn:syrianplains")))
					biomeCriteria = true;
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("minecrownreborn:syrianlands")))
					biomeCriteria = true;
				if (!biomeCriteria)
					continue;
				if (world.isRemote)
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("minecrownreborn", "great_watchtower"));
				if (template == null)
					return;
