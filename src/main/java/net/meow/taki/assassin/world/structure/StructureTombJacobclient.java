...
public class StructureTombJacobclient extends ElementsMinecrownrebornMod.ModElement {
	public StructureTombJacobclient(ElementsMinecrownrebornMod instance) {
		super(instance, 98);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("minecrownreborn", "jacob_tomb"));
				if (template == null)
					return;
