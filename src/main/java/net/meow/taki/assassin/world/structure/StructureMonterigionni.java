...
public class StructureMonterigionni extends ElementsMinecrownrebornMod.ModElement {
	public StructureMonterigionni(ElementsMinecrownrebornMod instance) {
		super(instance, 129);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 100) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("minecrownreborn", "monterrigioni_a.craft_version_1.00_with_entities"));
				if (template == null)
					return;
