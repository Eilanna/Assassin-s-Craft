...
public class StructureTombezior extends ElementsMinecrownrebornMod.ModElement {
	public StructureTombezior(ElementsMinecrownrebornMod instance) {
		super(instance, 93);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("minecrownreborn", "ezio_tomb_(relevations)"));
				if (template == null)
					return;
